...
export default function MoodPopup({
  ...
  }, [isOpen, initialMood, initialNote, selectedDate, entryId]);
  ...
  // Handle saving/updating mood entry
  const handleSave = async () => {
    ...
    try {
      ...
      // Call the callback with the updated/added mood data
      onSaved(selectedDate, selectedMood, trimmedNote, isUpdate);
      onClose();
    } catch (err) {
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          >
            {isUpdate
              ? `Update Feeling for ${formattedDate}`
              : `Add Mood for ${formattedDate}`}
          </motion.h3>
